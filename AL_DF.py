...
logging.basicConfig(filename='deepfool.log', filemode='a', format='%(name)s - %(levelname)s - %(message)s')
...
class Args:
    ...
    kwargs = {"batch_size": 64, "nb_grads": 5, "epsilon": 1e-04} # deepfool
...
logger.info("Average LPIPS score of original adversarial attack: ", orig_lpips.flatten().mean())
logger.info("Average LPIPS score of modifed adversarial attack: ", modf_lpips.flatten().mean())
...
logger.info(f"Original HM: {orig_hm}, Modified HM: {modf_hm}")

orig_linf = torch.max(torch.abs(xs - img_orig.to(device)))
modf_linf = torch.max(torch.abs(xs - img_modf.to(device)))

logger.info("Average Linf distance between original and original adversarial images: ", orig_linf.mean())
logger.info("Average Linf distance between original and modified adversarial images: ", modf_linf.mean())

orig_l2 = torch.cdist(xs, img_orig.to(device), p=2)
modf_l2 = torch.cdist(xs, img_modf.to(device), p=2)
logger.info("Average L2 distance between original and original adversarial images: ", orig_l2.mean())
logger.info("Average L2 distance between original and modified adversarial images: ", modf_l2.mean())
